...
4. `npiperelay.exe` をダウンロード（1Password SSH Agent ブリッジ用）
5. home-manager switch を実行
6. Claude CLI をインストール（`npm install -g @anthropic-ai/claude-code`）
7. Gemini CLI をインストール（`npm install -g @google/gemini-cli`）
...
| 1Password アイテム名 | エクスポート先 |
|---|---|
| `github.com` | `~/.ssh/imported_keys/github.com.pub` |
| `rouzinkai` | `~/.ssh/imported_keys/rouzinkai.pub` |
