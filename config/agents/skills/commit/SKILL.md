...
`Co-authored-by` には、実行しているあなた自身（AIエージェント）の名前と適切なドメインを使用すること。
...
Co-Authored-By: [あなたのエージェント名] <noreply@[あなたのエージェントのドメイン]>
```
...
コミットメッセージをテキストで提示した後、`AskUserQuestion` ツールを使って以下の形式で確認する：

- 質問：「この内容でコミットしますか？」
- header：「確認」
- 選択肢：「承認してコミット」（推奨）、「メッセージを修正する」
