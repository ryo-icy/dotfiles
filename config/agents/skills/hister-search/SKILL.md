...
- hister MCPサーバー（`http://127.0.0.1:4433/mcp`）が利用可能なエージェント（現状はClaude Codeに`--scope user`で登録済み。Antigravity CLI等もMCP対応のため今後登録され得る）では、`search`/`get_preview`/`get_history`のMCPツールを優先し、このスキルのシェルコマンドは使わない。MCPが登録されていないエージェントでのみ以下の手順に従う。
