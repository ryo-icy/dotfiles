{ lib, ... }: {
  ...
  # Requires: socat (in packages.nix) and ~/bin/npiperelay.exe (installed by bootstrap.sh).
  home.sessionVariables.SSH_AUTH_SOCK = "/tmp/ssh-agent-1p.sock";

  programs.zsh.initContent = lib.mkAfter ''
    _1P_SOCK="/tmp/ssh-agent-1p.sock"
    _RELAY="$HOME/bin/npiperelay.exe"
  ...
  # Ensure ~/bin (npiperelay.exe) and ~/.local/bin (claude CLI) are on PATH
  home.sessionPath = [
    "$HOME/bin"
    "$HOME/.local/bin"
  ];
